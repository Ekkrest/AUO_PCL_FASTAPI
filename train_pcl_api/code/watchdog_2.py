...
def complete_train_function(job_list, job_id):
    ...
    # move the models
    model_path = os.path.join("./records", str(job_id), "best.pth.tar")
    shutil.copy2(model_path, job_path)
    
    # save test path to yaml (for inference.sh)
    with open("./config/config.yaml", 'r') as rfile:
    ...
    modelcfg_path = os.path.join("./records", str(job_id), "config.yaml")
    ...
    # modellog_path = os.path.join("./records", str(job_id), "logger.log")
...
if __name__ == "__main__":
    ...
    if task_type == "train":
        pid = subprocess.Popen(["sh", "./run.sh", str(args.j_id), "&"], shell=False)
    else:
        pid = subprocess.Popen(["sh", "./inference.sh", str(args.j_id), "&"], shell=False)
